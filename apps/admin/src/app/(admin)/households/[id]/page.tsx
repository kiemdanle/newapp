...
export default async function HouseholdDetailPage({
  ...
  if (!household) return <p className="text-muted-foreground py-8 text-center">Household not found.</p>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <Link href="/households" className="text-sm text-muted-foreground hover:underline">
            ← Households
          </Link>
          <h1 className="text-xl font-semibold mt-1">{household.name}</h1>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p>
              ...
              <span className="text-muted-foreground">{household.ownerEmail}</span>
            </p>
          </div>
          <div>
