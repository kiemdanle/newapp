...
export const serverAdminApi = {
  ...
  settings: {
    ...
    photoLimits: {
      get: () =>
        apiServerFetch('/v1/admin/settings/photo-limits').then((r) =>
          photoLimitsSettingsSchema.parse(r),
        ),
      patch: (body: PhotoLimitsSettings) =>
        apiServerFetch('/v1/admin/settings/photo-limits', {
          method: 'PATCH',
          body,
