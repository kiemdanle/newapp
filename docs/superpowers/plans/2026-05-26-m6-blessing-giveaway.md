...
**Spec reference:** `docs/superpowers/specs/2026-05-23-expyrico-app-design.md` §2.13 (blessing/giveaway), §2.11 (offline policy — giveaways are online-only like reviews), §13 M6. Read before starting.
...
```bash
pnpm --filter @expyrico/api exec prisma migrate dev --name giveaways
```
...
```bash
psql postgresql://expyrico:expyrico@localhost:5432/expyrico -c "\dt giveaway*"
psql postgresql://expyrico:expyrico@localhost:5432/expyrico -c "\dt transaction_ratings notification_outbox"
psql postgresql://expyrico:expyrico@localhost:5432/expyrico -c "SELECT column_name FROM information_schema.columns WHERE table_name='users' AND (column_name LIKE '%rating%' OR column_name='transaction_count');"
psql postgresql://expyrico:expyrico@localhost:5432/expyrico -c "\d+ giveaway_claims"
```
...
```bash
pnpm --filter @expyrico/api exec prisma migrate dev --name report_target_giveaway
```
...
```bash
psql postgresql://expyrico:expyrico@localhost:5432/expyrico -c "SELECT enum_range(NULL::\"ReportTargetType\");"
```
...
### Task A4: Zod schemas in `@expyrico/shared`
...
- [ ] **Step 2: Run, verify FAIL** — `pnpm --filter @expyrico/api exec vitest run tests/unit/giveaway-state-machine.test.ts` (module not found).
...
- [ ] **API unit + integration:** `pnpm --filter @expyrico/api test` — all green, including the new state-machine, ratings, reputation-recompute, feed, create, claims, select/hand-off/confirm, ratings, rate-limit, user-reputation, and reports-giveaway suites.
- [ ] **Shared typecheck:** `pnpm --filter @expyrico/shared typecheck` — exit 0.
- [ ] **API typecheck + lint:** `pnpm --filter @expyrico/api typecheck && pnpm --filter @expyrico/api lint` — exit 0.
...
- [ ] **Mobile component tests:** `pnpm --filter @expyrico/mobile test` — `GiveawayCard`, `ClaimButton`, `TransactionRatingForm` pass.
- [ ] **Mobile E2E:** `maestro test apps/mobile/.maestro/giveaway-flow.yaml` — passes against a dev build (full two-phase handoff).
- [ ] **Admin E2E:** `pnpm --filter @expyrico/admin test:e2e` — giveaway moderation spec passes.
