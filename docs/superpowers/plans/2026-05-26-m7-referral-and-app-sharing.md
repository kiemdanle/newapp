# M7 — Referral Rewards + App Sharing Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship referral attribution with self-contained in-app rewards — points + badges, no real-money payouts. New users sign up carrying a referral **code**: typed in manually, or pre-filled from a code captured from an inbound link (best-effort, post-install) — there is **no auto-deep-link install-attribution claim** (see Red Team Review fix 3). When a referred user reaches a meaningful milestone (email verified AND first record created — whether that record arrives via the direct create path OR the offline-sync ingest path), a conversion fires: the referrer earns points via an append-only ledger and any newly-crossed badge thresholds are awarded, subject to a per-day velocity cap. A native share sheet lets users invite friends over WhatsApp/Telegram/SMS/etc. with the referral code (plus the URL as plain text). Mobile gets an Invite screen plus points/badges display; admin gets a light referrals/points overview with abuse flags (high-velocity + IP clustering).

**Architecture:** Three new Postgres tables (`referrals`, `points_ledger`, `user_badges`) plus a seeded catalog table (`badges`), and four additive columns on `users` (`referral_code`, `points_balance`, `referred_by_user_id`, plus the FK self-relation), all via a single Prisma migration. A pure-function `referral-code.ts` generator (short, URL-safe, collision-checked) with unit tests. A pure `badges.ts` evaluator (threshold crossing) with unit tests. A `referral-service.ts` that performs conversion in one transaction (mark referral converted → award points via the shared `awardPoints` helper → award badges) and is **idempotent** (one conversion per referred user), with a **per-referrer-per-day velocity cap** that suppresses points/badges beyond the cap. A single **`awardPoints(userId, delta, reason, refId, tx)`** helper writes the `points_ledger` row AND increments `points_balance` atomically — the only path that mutates points — guarded by an invariant test (`sum(ledger.delta) == points_balance`). Conversion is hooked on **both** the M1 direct first-record path **and** the M1 offline-sync ingest path, plus a reconciliation re-check, so no new BullMQ worker is required. Fastify routes under `/v1/me/referrals`, `/v1/me/points`, `/v1/badges`, and an **additive, optional** `referralCode` field on the existing `POST /v1/auth/register`. Mobile: an Invite screen using the OS share sheet (`expo-sharing` / RN `Share`) and the M0c `DeepLinkHandler` extended to best-effort capture an inbound `?code=` (post-install) and persist it (expo-secure-store) to pre-fill at sign-up; the code is also user-editable so manual entry always works. Admin: a Server-Component overview page over `serverAdminApi`, audit-logged where it mutates.
...
**Spec reference:** `docs/superpowers/specs/2026-05-23-pantry-app-design.md` §2.14 (referral rewards + app sharing) and §13 M7. Read before starting.
...
```bash
pnpm --filter @pantry/api exec prisma migrate dev --name referrals_points_badges
```
...
psql postgresql://pantry:pantry@localhost:5432/pantry -c "\di referrals*"
psql postgresql://pantry:pantry@localhost:5432/pantry -c "\d users" | grep -E "referral_code|points_balance|referred_by_user_id"
```
...
```bash
psql postgresql://pantry:pantry@localhost:5432/pantry -c "SELECT key, threshold FROM badges ORDER BY threshold;"
```
...
### Task A6: Zod schemas + error codes in `@pantry/shared`
...
```bash
pnpm --filter @pantry/api exec vitest run tests/unit/referral-code.test.ts
```
...
```bash
pnpm --filter @pantry/api exec vitest run tests/unit/badges.test.ts
```
Expected: FAIL — module not found.
...
import type { Badge as ApiBadge, PointsEntry, PointsReason, ReferredUser } from '@pantry/shared';
import { getPrisma } from '../../db.js';
...
> **Decision D4:** `getConfig().publicWebBaseUrl` is the base for the **plain-text** share URL (e.g. `https://pantry.app`). If M0a's config does not already expose it, add it there as part of this task (env `PUBLIC_WEB_BASE_URL`, default `https://pantry.app`). The share message includes both the **code** (the load-bearing part — the invited user enters it at sign-up) and this URL as readable text. **This is NOT a universal/app link** — v1.x provisions no AASA / `assetlinks.json` / served `/invite` page (see Red Team Review fix 3), so the URL does not auto-open the app or auto-attribute installs; attribution comes from the code. If the domain is unserved a tapped link simply 404s in a browser, which is acceptable for v1.x. Provisioning real universal links is a documented future addition (Handoff item 6).
...
```bash
pnpm --filter @pantry/api exec vitest run tests/integration/referral-conversion.test.ts
```
Expected: FAIL — module not found.
...
```bash
pnpm --filter @pantry/api exec vitest run tests/integration/register-referral.test.ts
```
...
import { referralSummarySchema } from '@pantry/shared';
import { getPrisma } from '../../db.js';
import {
  ensureReferralCode,
  shareUrlForCode,
...
```ts
import type { FastifyInstance } from 'fastify';
import { pointsListQuerySchema, pointsPageSchema } from '@pantry/shared';
import { getPrisma } from '../../db.js';
...
```ts
import type { FastifyInstance } from 'fastify';
import { badgeSchema } from '@pantry/shared';
import { z } from 'zod';
import { getPrisma } from '../../db.js';
...
```bash
pnpm --filter @pantry/api exec vitest run tests/integration/my-referrals.test.ts
```
Expected: 2 passed.
...
```bash
pnpm --filter @pantry/api exec vitest run tests/integration/my-points.test.ts tests/integration/badges-catalog.test.ts
```
Expected: 2 + 2 passed.
...
```bash
pnpm --filter @pantry/api exec vitest run tests/integration/admin-referrals-overview.test.ts
```
Expected: 2 passed.
...
In the existing URL-handling callback of `DeepLinkHandler` (which already parses `pantry://` links via Expo Linking), add: when the parsed path is `invite` and there is a `code` query param, call `capturePendingReferralCode(code)`. Do NOT change existing routing behavior — capture is additive and side-effect-only.

> **Scope (Red Team Review fix 3):** this only fires when the app is ALREADY installed and the link opens through it (e.g. a `pantry://invite?code=…` link, or a future universal link). v1.x provisions **no** universal/app-link infra (no AASA, no `assetlinks.json`, no served `/invite` page), so a fresh user tapping the `https` invite link in WhatsApp/Telegram **before install** will NOT auto-open the app or auto-attach the code — they install from the store and **type the code at sign-up**. This handler is a best-effort convenience for the already-installed case, not the primary attribution mechanism.
...
import type { ReferralSummary, PointsPage } from '@pantry/shared';
...
it('invokes the native share sheet with the referral CODE (the load-bearing part)', () => {
  ...
  const { getByText } = render(
    <InviteShareButton shareUrl="https://pantry.app/invite?code=ABCDEF23" code="ABCDEF23" />,
  );
...
export function InviteShareButton({ shareUrl, code }: { shareUrl: string; code: string }) {
  ...
  async function onShare() {
    ...
    await Share.share({
      message: `Join me on Pantry! Enter my invite code ${code} when you sign up. ${shareUrl}`,
...
```bash
pnpm --filter @pantry/mobile test -- InviteShareButton ReferralCodeCard PointsBadgesPanel
```
...
- [ ] **Step 1: Extend `serverAdminApi`** (`apps/admin/src/lib/admin-api.ts`) with a `referrals.overview()` method calling `GET /v1/admin/referrals/overview` and typed as `AdminReferralOverview` from `@pantry/shared`.
...
In one terminal: `pnpm --filter @pantry/api dev`. In another, register referrer, grab its code, register a referred user with that code, then hit the read endpoints:
