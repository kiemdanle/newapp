...
**Spec reference:** `docs/superpowers/specs/2026-05-23-pantry-app-design.md` §2.12 (deal sharing), §2.7 (voting), §2.8 (reporting/auto-hide), §13 M5. Read before starting.
...
- **Phase A — Data model** (Prisma schema, migrations, test harness/factories, `@pantry/shared` Zod schemas)
...
```bash
pnpm --filter @pantry/api exec prisma migrate dev --name deals_and_deal_votes
```
...
```bash
psql postgresql://pantry:pantry@localhost:5432/pantry -c "\dt deals*"
psql postgresql://pantry:pantry@localhost:5432/pantry -c "\dt deal_votes*"
psql postgresql://pantry:pantry@localhost:5432/pantry -c "\di deals*"
psql postgresql://pantry:pantry@localhost:5432/pantry -c "SELECT enum_range(NULL::report_target_type);"
```
...
### Task A5: Zod schemas in `@pantry/shared`
...
export const DEAL_PHOTO_CDN_HOST = 'cdn.pantry.app';
...
(If `maybeAutoHide` is typed to M2's `ReportTargetType` union from `@pantry/shared`, the added `'deal'` member from Task A5 makes this branch typecheck.)
...
- [ ] **Step 2: Verify FAIL** (`pnpm --filter @pantry/api exec vitest run tests/integration/deals-feed.test.ts` → 404, route not mounted).
...
import { dealListQuerySchema } from '@pantry/shared';
...
import type { Deal, DealCreate, DealPatch, DealSort } from '@pantry/shared';
...
- [ ] **Step 2: Typecheck** (`pnpm --filter @pantry/mobile typecheck` → exit 0).
...
import type { Deal } from '@pantry/shared';
...
import type { Deal, DealSort } from '@pantry/shared';
...
- [ ] **Step 5: Typecheck** (`pnpm --filter @pantry/mobile typecheck` → exit 0).
...
pnpm --filter @pantry/admin exec playwright test deals-moderation
```
