...
3. **Giveaway Cancel Lock Order & Projection**: Projected `user_id AS "userId"` in raw query; locked giveaway row `FOR UPDATE` first (serializing double-cancels), then quota, then record `FOR UPDATE`; asserted quota before restoring consumed items; added double-cancel and deadlock integration tests.
...
12. **Prisma Composite Seek Query AND-Composition & Resumable Drainage**: Structured Prisma query with `AND: [seekCondition, visibilityCondition]` to prevent silent `OR` key overwrites; eliminated arbitrary 20-page cap, implementing persistent resumable drainage checkpointing (`pantry.resumableSync.v1`) to drain arbitrary volumes without loops or restarts.
