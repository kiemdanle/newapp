...
## Overview
Integrate the SWR offline-first disk cache across all visual image components in `apps/mobile`. Replaces blank placeholders and loading flashes with instantaneous local cache rendering on mount across Pantry records, product catalog, giveaways, deals, and user profile screens.

<!-- Updated: Validation Session 2026-08-27 - Universal Frame-0 Cached Rendering across Components -->
...
- **Functional**:
  ...
  - `PrivateProductImage.tsx`:
    - Persist user draft/edit photos to disk cache; display immediately on app open.
  - `Avatar.tsx`:
...
|  - Frame 0: Instant cached URI                                  |
|  - Background: Conditional SWR Check (304 / 200)                |
+-----------------------------------------------------------------+
                                |
                                v
+-----------------------------------------------------------------+
|                  ImageDiskCache (L1 + L2)                       |
+-----------------------------------------------------------------+
```
...
1. Update `ProductThumbnail.tsx`:
   - Use `useCachedImage` to resolve candidate photo URIs.
