...
| F03 | P1 | Giveaway cancellation lacked `user_id` projection, omitted giveaway row lock, and inverted lock order | `api/src/routes/giveaways/cancel.ts:23-24`, `select.ts:37-55` | Projected `user_id AS "userId"`; locked giveaway row `FOR UPDATE` first (serializing double-cancels), then quota (`lockUserPantryQuota`), then record `FOR UPDATE`; asserted quota before restoring consumed item; added double-cancel and deadlock integration tests. |
