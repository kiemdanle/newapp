...
- [ ] Reorder exact unique set in one transaction using collision-safe temporary positions; delete relation/reindex/version and insert the `MediaOperationOutbox` cleanup row in the same transaction. A post-commit BullMQ publish may wake the worker, but polling/claiming the outbox guarantees progress if the process dies before enqueue.
