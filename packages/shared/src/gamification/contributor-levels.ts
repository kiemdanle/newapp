...
export const EXPYRICO_BADGE_COLORS: Record<ExpyricoBadgeColorToken, string> = {
  fresh_sage: '#4BAE8A',
  deep_sage: '#3A8F6F',
  mint_mist: '#D6F0E6',
  honey: '#F5A623',
...
export const FALLBACK_CONTRIBUTOR_TIER: ContributorLevelTier = {
  level: 1,
  title: 'Novice Scout',
  productsReq: 1,
  minPoints: 10,
...
export const DEFAULT_CONTRIBUTOR_LEVELS: readonly ContributorLevelTier[] = [
...
export function calculateContributorPoints(stats: ContributorStatsInput): number {
...
export function computeContributorProgression(
  stats: ContributorStatsInput,
  customLevels?: readonly ContributorLevelTier[],
): ContributorProgression {
  ...
  const firstTier: ContributorLevelTier = levels[0] ?? FALLBACK_CONTRIBUTOR_TIER;
  ...
  if (currentTierIndex === -1) {
    const nextTier: ContributorLevelTier = firstTier;
  ...
  const currentTier: ContributorLevelTier = levels[currentTierIndex] ?? FALLBACK_CONTRIBUTOR_TIER;
  ...
  const nextTier: ContributorLevelTier = levels[currentTierIndex + 1] ?? currentTier;
  const pointsToNext = Math.max(0, nextTier.minPoints - totalPoints);
  const productsToNext = Math.max(0, nextTier.productsReq - activeProducts);
