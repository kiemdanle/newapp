...
export const adminProductRowSchema = z.object({
  ...
  // — the admin client had to graft a *different* product's version (and
  // description) onto this row via a second request against the general
  // product-get route, which silently broke on a `merged_into` row (that
  // route resolves merge chains to the canonical product before returning —
  // a different id/version/description than this row's own). Both fields now
  // always describe *this* row directly.
...
export type AdminProductDeleteQuery = z.infer<typeof adminProductDeleteQuerySchema>;
...
export const productEditRecoverRequestSchema = productEditRecoverRequestUnionSchema.refine(
  (d) =>
    d.action !== 'rebase' ||
    new Set(d.desiredPhotoOrder.map((e) => (e.type === 'retained' ? e.sourceProductPhotoId : e.editPhotoId))).size ===
      d.desiredPhotoOrder.length,
  { message: 'desiredPhotoOrder entries must be unique' },
);
